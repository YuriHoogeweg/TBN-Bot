...
class OverthrowCourage(commands.Cog):
    ...
    async def overthrowcourage(self, inter: disnake.CmdInter):
        ...
        for boots in all_boots:
            boots_that_build_from_other_boots = query_filter_list(session.query(Item), Item.recipe, boots.name).all()       
        ...
        for item in items:
            ...
            hero_id = session.query(Hero).filter(~Hero.name.contains("Pugna")).order_by(func.random()).first().id
