...
class Birthdays(ChatCompletionCog):
    def __init__(self, bot: commands.Bot):
        ...
        Include many emoji relevant to birthday celebrations such as 🎂, 🥳, 🎉, ❤. Make sure to format it neatly by starting a new paragraph for each user.
         If there are multiple users with birthdays on the same day, make sure to mention all of them in the announcement, but each of their paragraphs should be unique.         If there are multiple users with birthdays on the same day, make sure to mention all of them in the announcement, but each of their paragraphs should be unique.
        ...
        self.set_message_context(system_message, [user_message_1], [
                                 assistant_message_1])
    ...
    async def set_birthday(self, interaction: ApplicationCommandInteraction, birthday: str):
        ...
        """
        date = datetime.strptime(birthday if len(
            birthday) == 10 else birthday[0:5] + '/1900', birthday_input_format)
    ...
    async def upcoming_birthdays(self, interaction: ApplicationCommandInteraction):
        ...
        def is_upcoming_birthday(member: TbnMember):
            birthday = datetime(
                year=datetime.now().year, month=member.birthday.month, day=member.birthday.day)
    ...
    async def remove_birthday(self, interaction: ApplicationCommandInteraction):
        self.db_session.query(TbnMember).filter(
            TbnMember.id == interaction.author.id).first().birthday = None
        self.db_session.commit()
    ...
    async def show_birthday(self, interaction: ApplicationCommandInteraction):
        birthday_boi = self.db_session.query(TbnMember).filter(
            TbnMember.id == interaction.author.id).first()
    ...
    async def notify_birthdays(self):
        ...
        message = f"There are {len(birthday_bois)} birthdays today, their User IDs are {', '.join(
            [str(member.id) for member in birthday_bois])}. Please write the announcement."
        announcement = await self.get_response(message)

        if announcement == "":
            announcement = (
                f'Good morning gang, today is <@!{birthday_bois[0].id}>\'s birthday! Happy birthday <@!{
                    birthday_bois[0].id}>! :partying_face: :birthday: :partying_face:'
                if len(birthday_bois) == 1
