        ...
        for x in range(0, 30):
            try:
                ...
                embed.add_field(name=event['Session1'],
                                value=convert_discord_localized_timestamp(event['Session1Date']),
                                inline=False)
                embed.add_field(name=event['Session2'],
                                value=convert_discord_localized_timestamp(event['Session2Date']),
                                inline=False)
                embed.add_field(name=event['Session3'],
                                value=convert_discord_localized_timestamp(event['Session3Date']),
                                inline=False)
                embed.add_field(name=event['Session4'],
                                value=convert_discord_localized_timestamp(event['Session4Date']),
                                inline=False)
                embed.add_field(name=event['Session5'],
                                value=convert_discord_localized_timestamp(event['Session5Date']),
                                inline=False)
...
def convert_discord_localized_timestamp(dt):
    return f"<t:{round(time.mktime(time.strptime(str(dt), '%Y-%m-%d %H:%M:%S')))}:f>"
